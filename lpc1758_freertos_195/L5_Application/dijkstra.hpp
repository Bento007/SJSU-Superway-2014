...
void setEdge( gNode *edgeWeight,uint32_t weight,
        uint32_t ticks, track_t type, uint32_t source,
        uint32_t destination, bool station, bool merge, bool fork,
        bool right)
{
    edgeWeight->weight= weight;
    edgeWeight->ticks = ticks;
    edgeWeight->type = type;
    edgeWeight->name.source = source;
    edgeWeight->name.destination = destination;
    edgeWeight->station= station;
    edgeWeight->merge= merge;
    edgeWeight->fork= fork;
    edgeWeight->right= right;
}
...
void makeGraph(struct dijkstra* graph)
{
            ...
            // F4 TO S1
            setEdge(&graph->edgeWeight[11][1], 2,2,fork,11,1,false,false,true,true);
