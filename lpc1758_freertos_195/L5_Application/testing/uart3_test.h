...
#ifndef Uart2_TEST_H_
#define Uart2_TEST_H_
 ...
 * ===================================
 */
int Uart2_print_test(void);
int Uart2_scan_test(void);
bool snap_test(void);
int selection();
void lights();
...
int test_select()
{
    while(1)
    {
        ...
        switch (selection())
        {
            case 0: Uart2_print_test(); break;
            case 1: Uart2_scan_test(); break;
...
int Uart2_print_test(void)
{
    Uart2& SNAP = Uart2::getInstance();
    SNAP.init(19200,32,32);
    int a=1;
    char x='A';

    LE.setAll(0xFF);
    printf("Uart2 Write Test:\n");
...
void send_Update_Test()
{
    ...
    uint8_t status = 2;
    com.send_Update(loc,status,speed);
