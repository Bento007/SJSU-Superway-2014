...
/*************************************************************************/
static volatile bool m_locked = false;   ///< Simple protection for simultaneous access of data structures
static bool m_rpt_node = false;          ///< Flag if we participate in the mesh network
static uint8_t m_our_node_id = 1;        ///< Our Node ID
static uint8_t m_retry_count = 2;        ///< Number of retries for ACK packet
static mesh_driver_t m_driver = { 0 };   ///< Radio send/recv functions
static mesh_error_mask_t m_error_mask = mesh_err_none;

static char m_our_name[MESH_DATA_PAYLOAD_SIZE] = { 0 };        ///< Name of our name used for PING response
static mesh_rte_table_t m_rte_table[MESH_MAX_NODES];           ///< Our routing table entries
static mesh_pkt_history_t m_pkt_hist[MESH_MAX_NODES];          ///< Our packet history
static mesh_pkt_history_t *m_pkt_hist_wptr = &(m_pkt_hist[0]); ///< Packet history write pointer (circular write)
static mesh_pnd_pkt_t m_mesh_pnd_pkts[MESH_MAX_NODES];         ///< Pending packets of other mesh nodes
static mesh_pnd_pkt_t m_our_pnd_pkts[MESH_MAX_PEND_PKTS];      ///< Pending packets sent by us

static const uint8_t m_rte_tbl_size       = MESH_ARRAY_SIZEOF(m_rte_table);
static const uint8_t m_pkt_history_size   = MESH_ARRAY_SIZEOF(m_pkt_hist);
static const uint8_t m_mesh_pnd_pkts_size = MESH_ARRAY_SIZEOF(m_mesh_pnd_pkts);
static const uint8_t m_our_pnd_pkts_size  = MESH_ARRAY_SIZEOF(m_our_pnd_pkts);

#if MESH_USE_STATISTICS
static mesh_stats_t m_mesh_stats = { 0 };
#endif
...
static bool mesh_update_soft_timers(void)
{
    ...
    const bool ok = m_driver.get_timer(&time_now_ms, sizeof(time_now_ms));
    const uint32_t delta = (time_now_ms - s_prev_time_ms);

    s_prev_time_ms = time_now_ms;
    mesh_incr_soft_timers_for_arr(&m_mesh_pnd_pkts[0], m_mesh_pnd_pkts_size, delta);
    mesh_incr_soft_timers_for_arr(&m_our_pnd_pkts[0],  m_our_pnd_pkts_size,  delta);
    return ok;
}
...
static inline bool mesh_send_packet(mesh_packet_t *pkt)
{
    ...
    if (pkt->nwk.src == m_our_node_id) {
        m_mesh_stats.pkts_sent++;
    }
    else {
        m_mesh_stats.pkts_repeated++;
    }
    #endif

    MESH_DEBUG_PRINTF("SEND TO %i THRU %i MAX HOPS %i", pkt->nwk.dst, pkt->mac.dst, pkt->info.hop_count_max);
    pkt->mac.src = m_our_node_id;
    return (m_driver.radio_send((void*)pkt, sizeof(*pkt)));
}
...
static void mesh_send_retry_packet(mesh_packet_t *pkt)
{
    #if MESH_USE_STATISTICS
    if (pkt->nwk.src == m_our_node_id) {
        m_mesh_stats.pkts_retried++;
    }
    else {
        m_mesh_stats.pkts_retried_others++;
    }
    #endif
...
static mesh_rte_table_t* mesh_find_rte_tbl_entry(const uint8_t dst_id)
{
    ...
    for (i = 0; i < m_rte_tbl_size; i++) {
        if (dst_id == m_rte_table[i].dst) {
            entry = &m_rte_table[i];
            break;
        }
    }
...
static mesh_rte_table_t* mesh_get_rte_to_modify(const uint8_t dst_id)
{
    ...
    if (NULL == entry) {
        ...
        if (NULL == entry) {
            entry = &(m_rte_table[0]);
            lowest = entry->score;
            for (i = 1; i < m_rte_tbl_size; i++) {
                if (m_rte_table[i].score < lowest) {
                    lowest = m_rte_table[i].score;
                    entry = &(m_rte_table[i]);
                }
            }
...
static void mesh_pending_packets_add(const mesh_packet_t *pPkt, const uint8_t num_hops)
{
     ...
     */
    if (m_our_node_id == pPkt->nwk.src) {
        entry = mesh_get_pnd_pkt_slot(&m_our_pnd_pkts[0], m_our_pnd_pkts_size);
    }
    else {
        entry = mesh_get_pnd_pkt_slot(&m_mesh_pnd_pkts[0], m_mesh_pnd_pkts_size);
    ...
    entry->pkt         = *pPkt;
    entry->pkt.info.retries_rem = m_retry_count; /* DO THIS AFTER COPYING THE PACKET!!! */
            ...
            else if(pnd->timer_ms >= pnd->timeout_ms)
            {
                ...
                if (pnd->pkt.info.retries_rem > 0) {
                    ...
                    if (mesh_pkt_ack_rsp != pnd->pkt.info.pkt_type &&
                            pnd->pkt.nwk.src == m_our_node_id &&    /* Source was us */
                        ...
                        MESH_DEBUG_PRINTF("RETRY PKT AND DISC NEW RTE WITH NWK %i/%i",
                                          pnd->pkt.nwk.src, pnd->pkt.nwk.dst);
                        mesh_send_retry_packet(&(pnd->pkt));
                        pnd->pkt.info.retries_rem = m_retry_count; /* Reset retry count */
                    }
                    else {
...
{
    mesh_handle_pnd_pkts_for_arr(pRxPkt, &m_mesh_pnd_pkts[0], m_mesh_pnd_pkts_size);
    mesh_handle_pnd_pkts_for_arr(pRxPkt, &m_our_pnd_pkts[0],  m_our_pnd_pkts_size);
}
...
static void mesh_update_history_and_routing(const mesh_packet_t *pPkt, bool *duplicate, bool *is_retry_packet)
{
    ...
    for(i = 0; i < m_pkt_history_size; i++) {
        mesh_pkt_history_t *existing = &m_pkt_hist[i];
    ...
    if (!duplicate_packet)
    {
        *m_pkt_hist_wptr = new_pkt;
        /* if the pointer was at the last element, reset back to zero */
        if (m_pkt_hist_wptr == &(m_pkt_hist[m_pkt_history_size-1])) {
            m_pkt_hist_wptr = &(m_pkt_hist[0]);
        }
        else {
            m_pkt_hist_wptr++;
        }
         ...
         */
        if (pPkt->mac.src != pPkt->nwk.src && m_our_node_id != pPkt->nwk.src) {
...
static void mesh_handle_our_packet(mesh_packet_t *pPkt, const bool dup, const bool ack)
{
    ...
    if (!dup && pPkt->info.data_len > 0) {
        ...
        if (!m_driver.app_recv(pPkt, sizeof(*pPkt))) {
            m_error_mask |= mesh_err_app_recv;
        }
    }
    ...
    if (ack && mesh_pkt_ack == pPkt->info.pkt_type) {
        ...
        if (0 == pPkt->info.data_len) {
            const uint8_t size = sizeof(m_our_name) <= sizeof(pPkt->data) ?
                                 sizeof(m_our_name) :  sizeof(pPkt->data);
            mesh_send_ack(m_our_name, size, pPkt);
        }
#if MESH_USE_STATISTICS
        else if (!mesh_send_ack((char*)&m_mesh_stats, sizeof(m_mesh_stats), pPkt))
#else
        else
#endif
        {
            /* If we use statistics and packet send fails, it is probably due to
             * pay-load size too small for m_stats structure size, so re-send it.
             */
               ...
               const bool send_discovery_packet)
{
    ...
    memset(&m_our_pnd_pkts[0], 0, sizeof(m_our_pnd_pkts));
    memset(&m_mesh_pnd_pkts[0], 0, sizeof(m_mesh_pnd_pkts));
    memset(&m_rte_table[0], 0, sizeof(m_rte_table));
    memset(&m_pkt_hist[0], 0, sizeof(m_pkt_hist));
    #if MESH_USE_STATISTICS
    memset(&m_mesh_stats, 0, sizeof(m_mesh_stats));
    #endif

    m_our_node_id = id;
    m_rpt_node = is_rpt_node;
    m_driver = d;
    memset(m_our_name, 0, sizeof(m_our_name));
    strncpy(m_our_name, node_name, sizeof(m_our_name)-1);

    /* If init works, then send discovery packet if asked */
    status = m_driver.radio_init(NULL, 0);
...
void mesh_service(void)
{
    ...
    if (m_driver.radio_recv(&packet, sizeof(packet)))
    {
        //MESH_DEBUG_PRINTF("Rx PKT FROM %i THRU %i", packet.nwk.src, packet.mac.src);

        #if MESH_USE_STATISTICS
        m_mesh_stats.pkts_intercepted++;
        m_mesh_stats.rte_entries = mesh_get_num_routing_entries();
        #endif
        ...
        /* For single radio systems, we shouldn't ever get our own packet back */
             ...
             */
            else if (m_our_node_id == packet.nwk.src) {
                pMeshPacket = &packet;
            ...
            else if(MESH_BROADCAST_ADDR == packet.nwk.dst)
            {
                ...
                if (!m_driver.app_recv(&packet, sizeof(packet))) {
                    m_error_mask |= mesh_err_app_recv;
                }
            ...
            else if (m_our_node_id == packet.nwk.dst)
            {
            ...
            else if (m_rpt_node && packet.info.hop_count < packet.info.hop_count_max) {
                pMeshPacket = &packet;
                   ...
                   uint8_t num_ptrs, ...)
{
    ...
    if(MESH_ZERO_ADDR == dst || dst == m_our_node_id   ||   /* Invalid destination */
    ...
    pkt->info.retries_rem = m_retry_count;
    pkt->info.pkt_seq_num = mesh_get_next_seq_num();

    pkt->nwk.dst = dst;
    pkt->nwk.src = m_our_node_id;
    pkt->mac.src = m_our_node_id;
...
const mesh_rte_table_t* mesh_get_routing_entry(const uint8_t route_num)
{
    ...
    for (idx = 0; idx < m_rte_tbl_size; idx++) {
        if (MESH_ZERO_ADDR != m_rte_table[idx].dst) {
            if (route_num == found_entries) {
                entry = &m_rte_table[idx];
                break;
            }
            ++found_entries;
        }
...
uint8_t mesh_get_pnd_pkt_count(void)
{
    ...
    for (i = 0; i < m_mesh_pnd_pkts_size; i++) {
        if (MESH_ZERO_ADDR != m_mesh_pnd_pkts[i].pkt.nwk.dst) {
            ++count;
        }
    }

    for (i = 0; i < m_our_pnd_pkts_size; i++) {
        if (MESH_ZERO_ADDR != m_our_pnd_pkts[i].pkt.nwk.dst) {
            ++count;
        }
    }
...
{
    mesh_rte_table_t *e =  mesh_find_rte_tbl_entry(node_addr);
    uint32_t timeout = e ? (1 + e->num_hops) * m_retry_count * MESH_ACK_TIMEOUT_MS :
                           (m_retry_count * MESH_ACK_TIMEOUT_MS * MESH_RTE_DISCOVERY_HOPS);
    return timeout;
}
