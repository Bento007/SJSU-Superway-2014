...
bool producer::init(void)
{
    /* Create the queue, and share the handle using an ID (enum) or index */
    return addSharedObject(shared_SensorQueue, xQueueCreate(1, sizeof(int)));
}
bool producer::run(void *p)
{
     ...
     */
    int data = AS.getX();
    return xQueueSend(getSharedObject(shared_SensorQueue), &data, portMAX_DELAY);
}
