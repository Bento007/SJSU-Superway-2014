...
static eint3_entry_t *m_eint3_list_head = NULL; ///< Instance of list of EINTS
static const uint8_t m_port2_offset = 32;       ///< Offset of Port2
    ...
    while (e && status) {
        ...
        if (e->pin_mask & status) {
            ...
            if (e->pin_mask & UINT32_MAX) {
                ...
                LPC_GPIOINT->IO2IntClr = (e->pin_mask >> m_port2_offset);
            }
        }
        e = e->next;
    };
...
static void eint3_enable(uint8_t pin_num, eint_intr_t type, void_func_t func)
{
       ...
       )
    {
        ...
        e->next = m_eint3_list_head;
        m_eint3_list_head = e;

        /* PORT0 */
        if (pin_num < m_port2_offset) {
            if (eint_falling_edge == type) LPC_GPIOINT->IO0IntEnF |= e->pin_mask;
            else  LPC_GPIOINT->IO0IntEnR |= e->pin_mask;
        }
        /* PORT2 */
        else {
            if (eint_falling_edge == type) LPC_GPIOINT->IO2IntEnF |= (e->pin_mask >> m_port2_offset);
            else LPC_GPIOINT->IO2IntEnR |= (e->pin_mask >> m_port2_offset);
        }
...
{
    eint3_enable((pin_num & 0x1F) + m_port2_offset, type, func);
}
