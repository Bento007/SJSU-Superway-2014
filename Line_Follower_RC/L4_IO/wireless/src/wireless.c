...
static xQueueHandle m_rx_queue = NULL;     ///< Queue handle for RX queue
static xQueueHandle m_ack_queue = NULL;    ///< Queue handle for RX Ack packet
static xSemaphoreHandle m_nrf_activity_sem = NULL; ///< If FreeRTOS is running, we will not poll for nordic activity
...
static char wireless_get_queued_pkt(xQueueHandle qhandle, mesh_packet_t *pkt, const uint32_t timeout_ms)
{
    char ok = 0;
...
static void nrf_irq_callback(void)
{
    ...
    xSemaphoreGiveFromISR(m_nrf_activity_sem, &yieldRequired);
...
{
    return wireless_get_queued_pkt(m_rx_queue, pkt, timeout_ms);
}

char wireless_get_ack_pkt(mesh_packet_t *pkt, const uint32_t timeout_ms)
{
    return wireless_get_queued_pkt(m_ack_queue, pkt, timeout_ms);
}
...
void wireless_service(void)
{
    ...
    if (taskSCHEDULER_RUNNING == xTaskGetSchedulerState()) {
        if (!nordic_intr_signal()) {
            const portTickType blockTime = mesh_get_pnd_pkt_count() ? 1 : portMAX_DELAY;
            xSemaphoreTake(m_nrf_activity_sem, blockTime);
        }
        mesh_service();
    }
...
{
    if (NULL == m_rx_queue) {
        m_rx_queue = xQueueCreate(WIRELESS_RX_QUEUE_SIZE, MESH_PAYLOAD);
    }
    if (NULL == m_ack_queue) {
        m_ack_queue = xQueueCreate(1, MESH_PAYLOAD);
    }
    if (NULL == m_nrf_activity_sem) {
        vSemaphoreCreateBinary(m_nrf_activity_sem);
    }
    ...
    return (NULL != m_rx_queue && NULL != m_ack_queue && NULL != m_nrf_activity_sem);
}
...
static int nrf_driver_app_recv(void *p, int len)
{
    ...
    long task_woken = 0;
    const mesh_packet_t *pkt = (mesh_packet_t*) p;
    const xQueueHandle qhandle = (mesh_pkt_ack_rsp == pkt->info.pkt_type) ? m_ack_queue : m_rx_queue;
