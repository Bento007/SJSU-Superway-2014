 ...
 * FullCAN is hardware filtering of the CAN messages and the configured
 * messages go straight into the CAN RAM (separate 2K RAM).
 *
 ...
 *
 * When the CAN bus is initialized, we configure it to accept and acknowledge ALL CAN
 * messages regardless of the ID.  For the filtering, in addition to the FullCAN,
...
/// The CAN BUS type
typedef enum {
    can_inv = 0, ///< Invalid CAN number
    can1,        ///< CAN1 (HW numbers CAN1 as 1)
    can2,        ///< CAN2 (HW numbers CAN2 as 2)
    can_max      ///< Do not use
} can_t;

/// CAN function pointer type
typedef void (*can_void_func_t)(void);
 ...
 * @post  The CAN bus is initialized, and by default ALL can messages are accepted and
 *         acknowledged on the CAN Bus.  If specific messages are supposed to be accepted,
 *         please setup the CAN filter.
 */
...
typedef struct {
    ...
    uint16_t can_num : 3; ///< CAN controller number (1=CAN1, 2=CAN2)
...
typedef struct {
    ...
    uint32_t can_num : 3;   ///< CAN controller number (1=CAN1, 2=CAN2)
 ...
 *
 * To generate a disabled slot, just pass 0xFFFF (sid) as the message id,
 ...
 * TO DO Enabling fc_intr bit is not yet supported
*/
 ...
 * the pointer in memory where the FullCAN entry is stored in memory.
 ...
 * @param pMsg      The same message returned from CAN_fullcan_get_entry_ptr()
 * @param pMsgCopy  The copy of the message you wish to read the data to
