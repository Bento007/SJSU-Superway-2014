...
static void configure_flash_acceleration(unsigned int clock)
{
    ...
    const int const_val = 0x03A; /* Datasheet says : Must be 0x03A */
...
static sys_boot_t m_sys_boot_type = boot_unknown; // Store reboot type for anyone to query
static rtc_t m_rtc_boot_time = {0};

sys_boot_t sys_get_boot_type()
{
    return m_sys_boot_type;
}
rtc_t sys_get_boot_time()
{
    return m_rtc_boot_time;
}
