...
public:
	...
	bool init(void)
	{
		...
		LPC_GPIO0->FIODIR |= (1 << muxSelect);			// muxSelect output
	...
	//	eint3_enable_port2( rightspeed, eint_rising_edge , rightptr);
		...
		LE.setAll(0xF);		// let user know when init is finished
	...
	{
		/// MERGING ///
		LPC_GPIO0->FIOCLR = muxSelect;
		if (!(LPC_GPIO1->FIOPIN & (1 << lleft)) && !(LPC_GPIO1->FIOPIN & (1 << rright))){
			locationCounter++;
			printf("location = %i\n", locationCounter);
		}
		else if (!(LPC_GPIO1->FIOPIN & (1 << lleft))){
			LPC_GPIO0->FIOSET = muxSelect;		// change from RC control to code control
			leftMotor.set(stop);
			rightMotor.set(go);
			printf("merge left\n");
			delay_ms(500);
		}
		else if (!(LPC_GPIO1->FIOPIN & (1 << rright))){
			LPC_GPIO0->FIOSET = muxSelect;		// change from RC control to code control
			leftMotor.set(go);
			rightMotor.set(stop);
			printf("merge right\n");
		...
		LPC_GPIO0->FIOCLR = muxSelect;			// change from code control to RC control
		return -1;
	}

private:
	// pin layout //
	int leftspeed=6;		// P2.6
	int rightspeed=7;		// P2.7
	int lleft = 20;			// P1.20
	int left = 0;			// P0.0
	int middle = 23;		// P1.23
	int right = 1;			// P0.1
