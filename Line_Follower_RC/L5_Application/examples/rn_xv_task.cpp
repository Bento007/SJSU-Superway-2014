...
bool wifiTask::init()
{
    // Initialize at this baud rate, we'll change it if we cannot communicate
    // Wifi should be initted by using Uart Object rather than UartDev
    // otherwise PINSEL pins won't be initted
    // mWifi.init(mWifiBaudRate, WIFI_RXQ_SIZE, WIFI_TXQ_SIZE);

    addSharedObject(WIFI_SHR_OBJ, mHttpReqQueue);
    return true;
}
